...
export const get_movie_details = async (imdbID) => {
    try {
...
router.get('/movies/search', async (req, res) => {
    try {
        ...
        const movies = await Promise.all(data.Search.map(async (movie) => get_movie_details(movie.imdbID)));
        if (movies.length === 0) {
            res.sendStatus(404);
            return;
        }
...
router.post('/favorites/:imdbID', async (req, res) => {
    ...
    try {
        ...
        const combined_rating = (movie.Ratings.reduce((acc, rating) => {
